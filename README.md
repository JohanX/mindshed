...
- **Image Storage:** Cloudinary (recommended) or S3-compatible (R2, MinIO locally)
  - Video support (Epic 35): both adapters store and serve video. Only Cloudinary derives **poster frames** via the `so_auto` URL transform; the S3-compatible adapter returns `null` from `getVideoPosterUrl` and the UI renders a generic play-icon card at tile size. Lightbox playback is identical on both adapters. Public gallery surfaces (journey, result, dashboard) honor the same tile-size policy — no `<video>` element at thumbnail size anywhere. Cascade cleanup routes `mediaType: 'video'` end-to-end through `deleteStep` / `deleteProject` / `deleteHobby` (verified by `storage-cascade-video.spec.ts` against MinIO). See [`_bmad-output/planning-artifacts/architecture.md`](../_bmad-output/planning-artifacts/architecture.md) § "Image Storage Adapter — Video Methods" for the full pattern.
  - Cloudinary direct upload (Story 35.5 / FR138): VIDEO uploads in Cloudinary mode use a browser-direct POST to `api.cloudinary.com` via a server-signed signature, bypassing Vercel's ~4.5 MB serverless body limit. IMAGE uploads continue through the existing Server Action path (camera JPEGs fit comfortably under the limit). See [`_bmad-output/planning-artifacts/architecture.md`](../_bmad-output/planning-artifacts/architecture.md) § "Cloudinary Direct-Upload (Browser → Cloudinary, FR138)" for the dispatch rule and the Vercel edge-limit decision record.
