...
test.describe('BOM Basic CRUD', () => {
  ...
  test('can add, edit, and delete BOM rows; pill reflects state', async ({ page }) => {
    await page.goto(projectUrl)
    await page.waitForLoadState('networkidle')

    async function addBomRow(name: string, required: string, unit: string) {
      await page.getByRole('button', { name: /Add row/ }).click()
      const form = page.locator('form').filter({ has: page.getByLabel('Item name') })
      await form.getByLabel('Item name').fill(name)
      await form.getByLabel('Required', { exact: true }).fill(required)
      await form.getByLabel('Unit', { exact: true }).fill(unit)
      await form.getByRole('button', { name: /^Save$/ }).click()
      // Wait for the form to collapse (indicates server round-trip finished)
      await expect(form.getByLabel('Item name')).toHaveCount(0, { timeout: 5000 })
    }

    // Add first row
    await addBomRow('Kaolin', '500', 'g')
    await expect(page.getByText('Kaolin').first()).toBeVisible({ timeout: 5000 })
    await expect(page.getByText('1 item · ready')).toBeVisible()

    // Add second row
    await addBomRow('Silica', '200', 'g')
    await expect(page.getByText('Silica').first()).toBeVisible({ timeout: 5000 })
    ...
    await firstRowRequired.fill('1000')
    ...
    await page.goto(projectUrl)
    await page.waitForLoadState('networkidle')
    await expect(page.locator('table').getByLabel('Required quantity').first()).toHaveValue('1000')
    ...
    await expect(page.getByText('Silica')).toHaveCount(0)
    await expect(page.getByText('1 item · ready')).toBeVisible()
  })
